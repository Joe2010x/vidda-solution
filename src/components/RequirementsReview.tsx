...
import type { EnhancedRetrievalResult, TaskRequirementMapping, MatchedRequirementDetail } from "@/types/retrieval";
...
export default function RequirementsReview({
  ...
  const handleApprove = () => {
    ...
    const approvedMappings = taskRequirementMappings.map(mapping => ({
      ...mapping,
      requirements: mapping.requirements.filter(req => !removedRequirements.has(req.id)),
                      ...
                      {mapping.requirements.map((req) => (
                        <div
                          ...
                          className={`p-3 rounded-lg border ${
                            removedRequirements.has(req.id)
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              ...
                              <div className="text-xs text-gray-500 space-y-1">
                                <p><span className="font-medium">Article:</span> {req.article}</p>
                                {req.sourceExcerpt && (
                                  <p className="italic">&ldquo;{req.sourceExcerpt.substring(0, 100)}...&rdquo;</p>
                                )}
                                {req.relevanceReason && (
                                  <p><span className="font-medium">Why:</span> {req.relevanceReason}</p>
                                )}
                              </div>
                            </div>
                            <button
                              onClick={() => toggleRequirementRemoval(req.id)}
                              className={`ml-2 p-1 rounded ${
                                removedRequirements.has(req.id)
                                  ? 'bg-green-100 text-green-600'
                                  : 'bg-red-50 text-red-600 hover:bg-red-100'
                              }`}
                              title={removedRequirements.has(req.id) ? 'Restore requirement' : 'Remove requirement'}
                            >
                              {removedRequirements.has(req.id) ? (
